...
- **Body similarity, multi-modal** — `body-search` fuses **face + skeletal frame + silhouette curves + side projection (butt) + bust + recorded stats + height + build size** into one rank, or pick a lens with `--by overall|lookalike|body|frame|curves|stats` (**`lookalike`** = face-led "closest-looking actress"). Because the visual vectors are scale-free, soft **height** and **build** (BMI) terms restore absolute size; `--height-tol N` adds a hard stature band ("same shape *and* same height"), and `--hair <colour>` filters by hair
